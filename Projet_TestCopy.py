...
# Demander les détails de la nouvelle tâche à l'utilisateur
    ...
    query = "INSERT INTO tache (type_tache, titre, description, equipe, createur) VALUES (%s, %s, %s, %s,%s)"
...
def afficher_taches_equipe(administrateur):
    ...
    query = "SELECT id, titre, description, type_tache, etat_fin FROM tache WHERE equipe = %s"
...
def modifier_tache(administrateur):
    ...
    id_sql="SELECT id FROM tache WHERE equipe = %s"
    ...
    if tache_id in liste_id_taches:
        ...
        query = "SELECT * FROM tache WHERE id = %s"
        ...
        if tache:
            ...
            query = "UPDATE tache SET titre = %s, description = %s, type_tache = %s, etat_fin =%s WHERE id = %s"
    ...
    if tache_id in liste_id_taches:
        ...
        if tache:
            ...
            if confirmation == "oui":
                ...
                query = "DELETE FROM tache WHERE id = %s"
                cursor.execute(query, (tache_id,))
                conn.commit()  # Valider la transaction
...
def inscrire_utilisateur(administrateur):
    ...
    if utilisateur:
        ...
        query = "INSERT INTO utilisateur (username, password, email, type_compte, equipe) VALUES (%s, %s, %s, %s, %s)"
...
def afficher_mes_taches(utilisateur):
    ...
    # Sélectionner les tâches de l'équipe de l'administrateur
    query = "SELECT id, type_tache, titre, description, etat_fin FROM tache WHERE createur = %s"
    cursor.execute(query, (username,))
...
def modifier_ma_tache(utilisateur):
    ...
    id_sql="SELECT id FROM tache WHERE createur = %s"
...
def notifier_ma_tache(utilisateur):
    ...
    if tache_id in liste_id_taches:
        ...
        if tache:
            ...
            # Exécuter une requête SQL de mise à jour pour modifier les champs de la tâche
            query = "UPDATE tache SET  etat_fin =%s WHERE id = %s"
...
def mes_statistiques(utilisateur):
    ...
    query="SELECT id FROM tache WHERE createur = %s AND etat_fin=%s"
...
def statistiques(administrateur):
    ...
    query="SELECT id FROM tache WHERE etat_fin=%s"
...
def super_modifier_tache(administrateur):
    ...
    if tache_id in liste_id_taches:
        ...
        if tache:
            ...
            # Exécuter une requête SQL de mise à jour pour modifier les champs de la tâche
            query = "UPDATE tache SET titre = %s, description = %s, type_tache = %s, equipe=%s, etat_fin =%s WHERE id = %s"
